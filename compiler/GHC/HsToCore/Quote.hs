...
repE (HsUnboundVar _ uv)   = do
                               ...
                               sname <- repNameS occ
                               repUnboundVar sname
repE (XExpr (HsExpanded a _))
  = repE a  -- Important: when quoting code, we want to get what the user
            -- wrote (the first field of HsExpanded) not what GHC happened
            -- to expand it into (the second field)
